...
export default async function middleware(req: NextRequest) {
  ...
  const excludedRoute = excludedRoutes.find((route) => url.startsWith(route));

  if (excludedRoute) {
    // If it is an excluded route, return the response without running the middleware
  ...
  if (!sessionCookie) {
    ...
    response.cookies.set("session", "1"); // Set session cookie to expire after 10 minutes
    return response;
  } else {
    if (visitorIdCookie) {
